...
final class SectionBuffer : Buffer
{
	...
	size_t maxSize()
	{
		if (_buffer.empty()) return size_t.max;
		size_t leng = _buffer[_buffer.length - 1].length;
		if (leng == _sectionSize) return size_t.max;
		else {
	...
	override void rest(size_t size = 0){_rSize = size;}
	
	override @property size_t length()  const  {return _wSize;}
	@property size_t stectionSize(){return _sectionSize;}
	...
	@property  readSize() 		const {return _rSize;}
	@property  readCount() 	const {return _rSize / _sectionSize;}
	@property  readSite()  	const {return _rSize % _sectionSize;}
	@property  writeCount()  	const {return _wSize / _sectionSize;}
	@property  writeSite()  	const {return _wSize % _sectionSize;}
private:
	size_t _rSize;
	size_t _wSize;
	Array!(ubyte[]) _buffer;
