...
class HTTPHeader
{
	...
	@property HTTPHeaderType type() const {return _type; }
	@property void type(HTTPHeaderType type){_type = type; }
	// REQUEST
	@property HTTPMethod method() const {return _method;} 
	@property void method(HTTPMethod met){_method = met;}
	@property string methodString()  const  {return method_strings[_method];}
	@property bool upgrade() const {return _upgrade;} 


	@property int statusCode() const {return _statuCode;} // RESPONSE only
	@property void statusCode(int code){_statuCode = code;}

	@property httpVersion() const {return _hversion;}
	@property httpVersion(HTTPVersion ver){_hversion = ver;}
	@property requestString()  const  {return _queryString;}
	...
	@property const(string[string]) headerMap() const {return _header;}

	@property host() const {return _header["Host"];}
	...
	bool isInVaild()  const  {return (_method == HTTPMethod.HTTP_INVAILD && _statuCode == -1);} // 无效的
package:
	void clear()
	{
		_statuCode = -1;
	...
	@property void upgrade(bool up){_upgrade = up;} 
private:
