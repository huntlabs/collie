...
final class HTTPParser
{
	...
	@property isUpgrade(){return upgrade;}
	@property contentLength(){return content_length;}
	@property isChunked() {return (flags & HTTPParserFlags.F_CHUNKED) == 0 ? false : true;}
	//@property status() {return status_code;}
	@property error(){return http_errno;}
	@property errorString(){return error_string[http_errno];}
	@property methodCode() { return method;}
	@property methodString() { return method_strings[method];}
	@property major(){return http_major;} //版本号首位
	@property minor(){return http_minor;} //版本号末尾
	@property handleIng (){return _isHandle;}
	@property handleIng(bool handle){_isHandle = handle;}
	@property skipBody() {return _skipBody;}
	@property skipBody(bool skip) {return _skipBody = skip;}

	/** 回调函数指定 */
	@property onMessageBegin(CallBackNotify cback){_on_message_begin = cback;}
	@property onMessageComplete(CallBackNotify cback){_on_message_complete = cback; }
	@property onHeaderComplete(CallBackNotify cback){ _on_headers_complete = cback; }
	@property onChunkHeader(CallBackNotify cback){ _on_chunk_header = cback; }
	@property onChunkComplete(CallBackNotify cback){ _on_chunk_complete = cback; }
	@property onUrl(CallBackData cback){_on_url = cback;}
	@property onStatus(CallBackData cback){_on_status = cback;}
	@property onHeaderField(CallBackData cback){_on_header_field = cback;}
	@property onHeaderValue(CallBackData cback){_on_header_value = cback;}
	@property onBody(CallBackData cback){_on_body = cback;}

	void rest(HTTPParserType ty) 
	{
		type = ty;
		state  = (type == HTTPParserType.HTTP_REQUEST ? HTTPParserState.s_start_req : (type == HTTPParserType.HTTP_RESPONSE ? HTTPParserState.s_start_res : HTTPParserState.s_start_req_or_res));
...
public:
	...
	ulong httpParserExecute(ubyte[] data)
	{
		...
		reexecute:
			switch (state) {
				...
				case HTTPParserState.s_start_req:
				{
					...
					index = 1;
					switch (ch) {
						case 'A': method = HTTPMethod.HTTP_ACL; break;
						case 'B': method = HTTPMethod.HTTP_BIND; break;
						case 'C': method = HTTPMethod.HTTP_CONNECT; /* or COPY, CHECKOUT */ break;
						...
						case 'L': method = HTTPMethod.HTTP_LOCK; /* or LINK */ break;
						case 'M': method = HTTPMethod.HTTP_MKCOL; /* or MOVE, MKACTIVITY, MERGE, M-SEARCH, MKCALENDAR */ break;
						case 'N': method = HTTPMethod.HTTP_NOTIFY; break;
						case 'O': method = HTTPMethod.HTTP_OPTIONS; break;
						case 'P': method = HTTPMethod.HTTP_POST;
							/* or PROPFIND|PROPPATCH|PUT|PATCH|PURGE */
							break;
						case 'R': method = HTTPMethod.HTTP_REPORT; /* or REBIND */ break;
						case 'S': method = HTTPMethod.HTTP_SUBSCRIBE; /* or SEARCH */ break;
						case 'T': method = HTTPMethod.HTTP_TRACE; break;
						case 'U': method = HTTPMethod.HTTP_UNLOCK; /* or UNSUBSCRIBE, UNBIND, UNLINK */ break;
						default:
							//error("err0");
							http_errno = HTTPParserErrno.HPE_INVALID_METHOD;
							goto error;
				...
				case HTTPParserState.s_req_fragment:
				{
					switch (ch) {
						...
						case LF:
							...
							http_minor = 9;
							state = (ch == CR) ?
								HTTPParserState.s_req_line_almost_done :
					...
					for (; p < maxP; p++) {
						...
						switch (header_state) {
							...
							case HTTPParserHeaderstates.h_C:
								index++;
								header_state = (c == 'o' ? HTTPParserHeaderstates.h_CO : HTTPParserHeaderstates.h_general);
								break;
								
							case HTTPParserHeaderstates.h_CO:
								index++;
								header_state = (c == 'n' ? HTTPParserHeaderstates.h_CON : HTTPParserHeaderstates.h_general);
								break;
							...
							case HTTPParserHeaderstates.h_matching_proxy_connection:
								index++;
								if (index > PROXY_CONNECTION.length
							...
							case HTTPParserHeaderstates.h_matching_content_length:
								index++;
								if (index > CONTENT_LENGTH.length
							...
							case HTTPParserHeaderstates.h_matching_transfer_encoding:
								index++;
								if (index > TRANSFER_ENCODING.length
							...
							case HTTPParserHeaderstates.h_upgrade:
								if (ch != ' ') header_state = HTTPParserHeaderstates.h_general;
								break;
				...
				case HTTPParserState.s_header_value_start:
				{
					...
					switch (header_state) {
						...
						case HTTPParserHeaderstates.h_transfer_encoding:
							...
							if ('c' == c) {
								header_state = HTTPParserHeaderstates.h_matching_transfer_encoding_chunked;
							} else {
								header_state = HTTPParserHeaderstates.h_general;
							}
							break;
				...
				case HTTPParserState.s_header_value: //BUG，找不到结束
				{
					...
					for (; p < maxP; p++) {
						...
						if (!lenient_http_headers && !(ch == CR || ch == LF || ch == 9 || (ch > 31 && ch != 127))) {
							http_errno = HTTPParserErrno.HPE_INVALID_HEADER_TOKEN;
							goto error;
						}
						...
						switch (h_state) {
							...
							case HTTPParserHeaderstates.h_transfer_encoding_chunked:
								if (ch != ' ') h_state = HTTPParserHeaderstates.h_general;
								break;
				...
				case HTTPParserState.s_headers_almost_done:
				{
							...
							method == HTTPMethod.HTTP_CONNECT);
					{
						if(_on_headers_complete != null){
							...
							if(skipBody) flags |= HTTPParserFlags.F_SKIPBODY;
				...
				{
					ulong to_read = content_length <
						cast(ulong) (maxP - p) ? content_length : cast(ulong) (maxP - p);
		...
		assert(((header_field_mark != uint.max ? 1 : 0) +
				(header_value_mark  != uint.max ? 1 : 0) +
				(url_mark  != uint.max ? 1 : 0)  +
				(body_mark  != uint.max ? 1 : 0) +
...
protected:
	...
	bool http_message_needs_eof ()
	{
		...
		if (status_code / 100 == 1 || /* 1xx e.g. Continue */
			...
			flags & HTTPParserFlags.F_SKIPBODY) {     /* response to a HEAD request */
			return false;
		}
	...
	HTTPParserState parse_url_char(HTTPParserState s, ubyte ch)
	{
		...
		switch (s) {
			...
			case HTTPParserState.s_req_server:
			{
				...
				if (/*mixin(IS_USERINFO_CHAR("ch"))*/IS_USERINFO_CHAR2(ch) || ch == '[' || ch == ']') {
