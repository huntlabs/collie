...
string getComplierName(){
	...
	name = name ~ " / " ~ to!string(compiler.version_major) ~ "." ~ to!string(compiler.version_minor);
	name ~= ".";
...
class HTTPResponse
{
	...
	final void close()
	{
		if( !_done && fn_resClose)
			fn_resClose(this);
		_done = true;
	}

	final @property sentCall(CallBackResponse cback){fn_resDone = cback;}
	final @property closeCall(CallBackResponse cback){fn_resClose = cback;}
	...
	{	
		...
		resp._header.setHeaderValue("Content-Length",resp._body.length);
...
private:
	HTTPHeader _header;
	SectionBuffer _body;
	CallBackResponse fn_resDone;
	CallBackResponse fn_resClose;
...
private string statusText(int code) 
{
	switch (code)
	{
		...
		case 102 : return "Processing";												// RFC2518
		case 200 : return "OK";
		...
		case 207 : return "Multi-Status";											// RFC4918
		case 208 : return "Already Reported";									// RFC5842
		...
		case 308 : return "Permanent Redirect";							// RFC7238
		...
		case 425 : return "Reserved for WebDAV advanced collections expired proposal";   // RFC2817
		...
		case 431 : return "Request Header Fields Too Large";														// RFC6585
		...
		case 506 : return "Variant Also Negotiates (Experimental)";											// RFC2295
		case 507 : return "Insufficient Storage";																				// RFC4918
		case 508 : return "Loop Detected";																							// RFC5842
