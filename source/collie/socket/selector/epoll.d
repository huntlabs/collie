...
final class EpollLoop
{
	...
	{
		if(event.fd == socket_t.init) return false;
...
protected:
	void handleTCP(Object obj,uint events)
	{
...
protected:
	pragma(inline,true);
	bool isErro(uint events){return (events & (EPOLLHUP | EPOLLERR | EPOLLRDHUP)) != 0;}
	pragma(inline,true);
	bool isRead(uint events){return (events & EPOLLIN) != 0;}
	pragma(inline,true);
	bool isWrite(uint events){return (events & EPOLLOUT) != 0;}
