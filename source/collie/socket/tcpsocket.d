...
class TCPSocket : AsyncTransport , EventCallInterface
{
	...
	final override @property int fd(){return cast(int)_socket.handle();}
	...
	void write(ubyte[] data,TCPWriteCallBack cback)
	{
		...
		eventLoop.post(bind!(void delegate(WriteSite))(&doWrite,new WriteSite(data,cback))); //利用eventloop的post处理跨线程问题
	}
	...
	@property @trusted Address remoteAddress() {return _socket.remoteAddress();}

	final void setReadCallBack(TCPReadCallBack cback){ _readCallBack = cback;}

	final void setCloseCallBack(CallBack cback){_unActive = cback;}
...
mixin template TCPSocketOption()
{
	...
	@property @trusted Address localAddress() {return _socket.localAddress();}
}
...
final class WriteSite
{
	...
	@property data(){return _data[_site..$];}
