...
protected:
    ...
    {
        if(_now == 0) return (_list.length - 1);
        else return (_now -1);
    }
...
abstract class WheelTimer
{
    ...
    bool isActive() const nothrow {return _manger !is null;}
    
    @property oneShop(){ return _oneShop;}
    @property oneShop(bool one){_oneShop = one;}
private:
    WheelTimer _next;
    WheelTimer _prev;
    TimingWheel _manger;
...
class TestWheelTimer : WheelTimer
{
    ...
    {
        try{
        writeln("\nname is ", name ," \tcutterTime is : " ,Clock.currTime().toSimpleString() , "\t new time is : ", time.toSimpleString());
        } catch{}
    //       assert();
    }
