...
public:
  stats(size_t block_size,int timeout)
    : mutex_(),
      ...
      total_bytes_read_(0),block_size(block_size),timeout(timeout)
  {
  }
  ...
  void print()
  {
    ...
    std::cout << total_bytes_written_ / (1024 * 1024) / timeout << " MB/s written\n";
    std::cout << total_bytes_written_ / block_size / timeout << " QPS/s written\n\n";

    std::cout << total_bytes_read_ << " total bytes read\n";
    std::cout << total_bytes_read_ / (1024 * 1024) / timeout<< " MB/s read\n";
    std::cout << total_bytes_read_ / block_size / timeout<< " QPS/s read\n";
  }
